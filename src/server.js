...
async function handleRegisteredOperationRequest(db, request, response, hash, routes, trace = null) {
  ...
  const operation = await tracePhase(trace, 'registered-operation-lookup', () => operationForHash(db.config, hash), {
    hash,
  });
  if (!operation) {
    throw dbError(
  ...
  const restRequest = tracePhaseSync(trace, 'registered-operation-execution', () => operationRequest(operation, body?.variables ?? {}), {
    hash,
  });
