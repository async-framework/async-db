export const dashboardCsvReader = {
  name: 'csv-dashboard-csv',
  match({ file }) {
  ...
  async read({ readText, sourceFile }) {
    return {
      kind: 'data',
      format: 'csv',
      data: parseDashboardCsvRecords(await readText(), sourceFile),
    };
  },
};

function parseDashboardCsvRecords(text, filePath = 'CSV file') {
  const rows = parseDashboardCsvRows(text);
  const headerRow = rows.shift();

  if (!headerRow || headerRow.every((cell) => cell.value.trim() === '')) {
    throw new Error(`${filePath} must start with a header row.`);
  }
...
function parseDashboardCsvRows(text) {
  const rows = [];
  let row = [];
  let field = '';
  let quoted = false;
...
function fieldNameFromHeader(header, index) {
  ...
  if (/^[a-z_$][A-Za-z0-9_$]*$/u.test(trimmed)) {
    return trimmed;
  }
  ...
  const words = trimmed.match(/[A-Za-z0-9]+/gu) ?? [];
...
function isStringLikeHeader(headerName) {
  return /\b(id|uuid|guid|zip|postal|phone|tel|code)\b/iu.test(String(headerName).replaceAll(/[^A-Za-z0-9]+/gu, ' '));
}
